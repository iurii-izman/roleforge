...
- **Telegram:** [Telegram interaction spec](docs/specs/telegram-interaction.md). `roleforge.digest`, `roleforge.queue`, `roleforge.delivery_log` (log_telegram_delivery for `digest`, `queue_card`, `alert`, `admin_alert`, `batch`). Review actions in queue.apply_review_action. Digest and queue UX in v2 include score bands, queue position, and short “why in queue” explanations.
...
- **Structured logging & admin alert:** [Structured logging](docs/specs/structured-logging.md) (JSON to stdout); [Admin alert path](docs/specs/admin-alert-path.md) (consecutive-failure Telegram alert to `TELEGRAM_ADMIN_CHAT_ID`).
- **Cost governance:** [Cost governance](docs/specs/cost-governance.md) — `job_runs.summary.ai_cost_usd` (TASK-104), monthly review query and checklist (TASK-105).
- **AI enrichment (v4):** [AI enrichment contract](docs/specs/ai-enrichment-contract.md) — provider/model, input/output, gating, timeout/retry/fallback, cost and prompt versioning (TASK-062); AI governance in [Architecture](docs/architecture.md) (TASK-067). Implementation: `roleforge.enrichment` (enrich_one, run_enrichment_for_high_scores, update_vacancy_ai_metadata), `roleforge.prompts.enrichment` (PROMPT_VERSION, build_user_prompt); schema `003_ai_metadata.sql`.
...
- **Market monitoring (v6):** [v6 market monitoring spec](docs/specs/v6-market-monitoring.md). File registry: `config/monitors.yaml`. Kill-switch: `MONITOR_INTAKE_ENABLED` (default off). `roleforge.monitor_registry`, `roleforge.monitors.hh`, `roleforge.jobs.monitor_poll`; HH.ru sources use the same normalize/dedup path and `monitor:hh:{id}` source keys.
- **Applications (v5):** [v5 application lifecycle](docs/specs/v5-application-lifecycle.md). Additive Postgres: `schema/004_application_lifecycle.sql`, `schema/005_gmail_classified.sql` (TASK-072). [Inbox classifier](docs/specs/inbox-classifier.md) (TASK-073): deterministic rules for `gmail_messages.classified_as`; implementation in `roleforge.inbox_classifier` (TASK-075). [AI inbox classification contract](docs/specs/ai-inbox-classification-contract.md) (TASK-074) defines optional AI fallback for ambiguous messages. Application state transitions (TASK-078): `roleforge.application_lifecycle.apply_application_transition`; Telegram handlers call it with application_id and target status; invalid transitions are rejected.
- **Web UI scope (v7):** [v7 web UI scope](docs/specs/v7-web-ui.md). Approved as a single-user operator console with Bearer token auth; Telegram remains the primary delivery and quick-action surface.
- **Runtime entrypoints:** `python -m roleforge.scheduler` (coordinated scheduled jobs), `python -m roleforge.jobs.gmail_poll`, `python -m roleforge.jobs.feed_poll`, `python -m roleforge.jobs.monitor_poll`, `python -m roleforge.jobs.replay`, `python -m roleforge.jobs.inbox_classify`, `python -m roleforge.jobs.employer_thread_match`, `python -m roleforge.jobs.interview_event_extract`, `python -m roleforge.jobs.application_notify --dry-run`, `python -m roleforge.jobs.interview_event_ai_enrich`, `python -m roleforge.jobs.digest --dry-run`, `python -m roleforge.jobs.queue --dry-run`, `python -m roleforge.jobs.alert --dry-run`, `python -m roleforge.jobs.batch --dry-run`. Helpers: `scripts/seed_default_profile.py`, `scripts/run_scoring_once.py`, `scripts/inspect_gmail_message.py`.
