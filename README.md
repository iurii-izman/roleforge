...
- **Telegram:** [Telegram interaction spec](docs/specs/telegram-interaction.md). `roleforge.digest`, `roleforge.queue`, `roleforge.delivery_log` (log_telegram_delivery for `digest`, `queue_card`, `alert`, `admin_alert`). Review actions in queue.apply_review_action. Digest and queue UX in v2 include score bands, queue position, and short “why in queue” explanations.
