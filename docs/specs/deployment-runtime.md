...
- **Process model**: a single containerized app image (or equivalent) that can run:
  ...
  - a scheduled queue-maintenance job,
...
- **Entry points** (suggested; exact commands live next to code, not here):
  ...
  - `python -m roleforge.jobs.queue` – Telegram review queue maintenance.
  - `python -m roleforge.jobs.replay` – manual replay helpers.
- **Scheduling**: done by the hosting platform (cron-like scheduler) or an external scheduler; RoleForge itself is stateless between runs and assumes Postgres as the source of truth.
