...
| 2026-03-18 | v6 salary modeling decision: keep `salary_raw` only, do not add `salary_structured` in the current roadmap | Accepted | Structured salary adds schema and scoring complexity without enough operator value yet; TASK-089 and TASK-090 are explicitly closed by scope decision, not implementation |
| 2026-03-18 | v7 web UI scope decision: single-user operator console, Bearer token auth, Telegram remains primary action surface | Accepted | docs/specs/v7-web-ui.md; web focuses first on analytics, health, source management, queue browser, and profile inspection/editing; application workspace is a later wave |
